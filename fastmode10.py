...
timer_started = False
font = pygame.font.SysFont("Arial", 24) # Font for the timer
...
def Game(): # Creates Game loop
    ...
    while game_screen:
        ...
        if remaining_orbs < 1: # If the count of orbs is 0 then it activates the win condition.
            ...
            display_sec = final_time
        else:
            display_sec = (pygame.time.get_ticks() - start_time) /1000
        ...
        timer_text = font.render(f"Time: {display_sec:.3f}s", True, (255, 255, 255)) # Renders the timer on the game screen
